...
This note continues from FCFS and introduces three important scheduling algorithms:
...
The note also compares **preemptive vs non-preemptive** versions. 
...
The first SJF in your lecture is **non-preemptive**. 
...
The lecture says the criteria for SJF involve:
...
Your lecture points out two important problems:
...
A process can potentially wait for a very long time if shorter jobs keep getting selected before it. The lecture explicitly notes that process starvation might happen. 
...
The lecture says priority is assigned when the process is created. 
...
The lecture gives the solution:
...
The lecture specifically says RR does not depend on BT for its selection criterion. 
